...
ERROR_CODE_APP_ID_INVALID = (
    1  # Invalid appID parameter when calling the method
)
ERROR_CODE_USER_ID_INVALID = (
    3  # Invalid userID parameter when calling the method
)
ERROR_CODE_SECRET_INVALID = (
    5  # Invalid secret parameter when calling the method
)
ERROR_CODE_EFFECTIVE_TIME_IN_SECONDS_INVALID = (
    6  # Invalid effective_time_in_seconds parameter when calling the method
)
...
def __aes_pkcs5_padding(cipher_text, block_size):
    padding_size = (
        len(cipher_text)
        if (len(cipher_text) == len(cipher_text.encode("utf-8")))
        else len(cipher_text.encode("utf-8"))
    )
...
def generate_token04(
    app_id, user_id, secret, effective_time_in_seconds, payload
):
    """Basic description
    ...
    if type(secret) != str or len(secret) != 32:
        return TokenInfo(
            "", ERROR_CODE_SECRET_INVALID, "secret must be a 32 byte string"
        )
    if (
        type(effective_time_in_seconds) != int
        or effective_time_in_seconds <= 0
    ):
        return TokenInfo(
            "",
            ERROR_CODE_EFFECTIVE_TIME_IN_SECONDS_INVALID,
            "effective_time_in_seconds invalid",
        )
